...
async function run() {
    ...
    const mode = process.env.INPUT_MODE === "verify" ? "verify" : "scan";
    ...
    if (Number.isFinite(minimum) && report.score < minimum)
        throw new Error(`Agent readiness ${report.score}/100 is below the required ${minimum}.`);
}
run().catch((error) => {
