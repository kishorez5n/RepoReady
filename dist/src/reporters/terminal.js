...
export function terminalReport(report) {
    const output = [
        "",
    ...
    for (const dimension of report.dimensions) {
        output.push(`  ${dimension.label.padEnd(30)} ${bar(dimension.score)} ${String(dimension.score).padStart(3)}/100  (${dimension.weight}%)`);
    }
    ...
    if (failures.length === 0)
        output.push("  No readiness deductions found.");
    for (const item of failures)
        output.push(finding(item));
    if (report.commands.length > 0) {
        output.push("", "Verification commands");
        for (const command of report.commands) {
            output.push(`  ${command.status.toUpperCase().padEnd(9)} ${(command.durationMs / 1000).toFixed(1).padStart(6)}s  ${command.command}`);
        }
    }
    output.push("", `${report.summary.passed} passed · ${report.summary.failed} failed · ${report.summary.skipped} skipped`, "");
    return output.join("\n");
}
