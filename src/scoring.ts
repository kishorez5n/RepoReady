import type { Category, DimensionScore, ReadinessReport, RuleResult, ScanMode, RepositoryProfile, CommandResult } from "./types.js";

const DIMENSIONS: Array<{ category: Category; label: string; weight: number }> = [
...
export function scoreResults(
  ...
  const dimensions: DimensionScore[] = DIMENSIONS.map((dimension) => {
    const applicable = results.filter(
      (item) => item.category === dimension.category && item.status !== "skip",
    );
  ...
  const caps = results
    ...
    .map((item) => item.scoreCap as number);
