...
function validateConfig(value: unknown, configPath: string): RepoReadyConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(`Invalid JSON object in Repo Ready configuration: ${configPath}`);
  }
  const raw = value as Record<string, unknown>;
  const validProfiles: RepositoryProfile[] = ["docs", "node", "python", "monorepo", "generic"];
  if (raw.profile !== undefined && !validProfiles.includes(raw.profile as RepositoryProfile)) {
  ...
  const commandEntries = raw.commands ? Object.entries(raw.commands as Record<string, unknown>) : [];
  ...
  return {
    ...(raw.profile ? { profile: raw.profile as RepositoryProfile } : {}),
    ...(commandEntries.length > 0 ? { commands: Object.fromEntries(commandEntries) as Record<string, string> } : {}),
...
function detectProfile(
  ...
  if (sourceFiles.length === 0 && markdownFiles.length > 0) return "docs";
  return "generic";
}

function makeCommandCatalog(
  manifest: PackageManifest | undefined,
...
export async function discoverRepository(
  ...
  const configPath = explicitConfigPath
    ...
    : defaultConfigPath;
  let config: RepoReadyConfig = {};
  if (await exists(configPath)) {
    const rawConfig = await readJson<unknown>(configPath);
    if (rawConfig === undefined) throw new Error(`Invalid JSON in Repo Ready configuration: ${configPath}`);
    config = validateConfig(rawConfig, configPath);
  }
  const discoveredFiles = await walkFiles(root);
  const files = discoveredFiles.filter(
    (file) => !(config.excludePaths ?? []).some((pattern) => matchesGlob(file, pattern)),
  );
  const fileSet = new Set(files);
  const packagePath = path.join(root, "package.json");
  const rawManifest = await readJson<Record<string, unknown>>(packagePath);
  ...
  if (rawManifest) {
    const rawScripts = rawManifest.scripts && typeof rawManifest.scripts === "object"
      ? Object.entries(rawManifest.scripts as Record<string, unknown>)
      : [];
    packageManifest = {
      scripts: Object.fromEntries(rawScripts.filter((entry): entry is [string, string] => typeof entry[1] === "string")),
      ...
      ...(rawManifest.engines && typeof rawManifest.engines === "object"
        ? { engines: rawManifest.engines as Record<string, string> }
        : {}),
    };
  }
...
export async function readRepositoryFile(context: ScanContext, relative: string): Promise<string> {
