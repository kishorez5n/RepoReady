...
function packageManagerCommand(context: ScanContext, script: string): string {
...
function selectCommands(context: ScanContext): Array<{ name: string; command: string }> {
  ...
  const selected: Array<{ name: string; command: string }> = [];
  for (const candidates of [
    ...
    if (!name || selected.some((item) => item.name === name)) continue;
...
async function executeCommand(
  ...
  return new Promise((resolve) => {
    ...
    child.stdout.on("data", (chunk: Buffer | string) => {
      ...
      if (stdout.length > 128_000) stdout = truncate(stdout);
    });
    child.stderr.on("data", (chunk: Buffer | string) => {
      stderr += chunk.toString();
      if (stderr.length > 128_000) stderr = truncate(stderr);
    });
...
export async function verifyRepository(
...
): Promise<{ commands: CommandResult[]; results: RuleResult[] }> {
  ...
  const results: RuleResult[] = commands.map((command, index) =>
