...
type Format = "terminal" | "json" | "html";
...
function parseArguments(args: string[]): ParsedArguments {
  ...
  if (args[0] === "scan" || args[0] === "verify") mode = args.shift() as ScanMode;
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--format") {
      ...
      if (!(["terminal", "json", "html"] as string[]).includes(value)) throw new Error(`Unsupported format: ${value}`);
    ...
    if (argument === "--min-score") {
      ...
      if (!Number.isFinite(minScore) || minScore < 0 || minScore > 100) throw new Error("--min-score must be a number from 0 through 100.");
      index += 1;
      continue;
    }
    if (argument === "--timeout") {
    ...
    if (positionalSeen) throw new Error(`Unexpected positional argument: ${argument}`);
    target = argument;
    positionalSeen = true;
  }
...
async function main(): Promise<void> {
  ...
  if (parsed.minScore !== undefined && report.score < parsed.minScore) process.exitCode = 2;
}
